...
export const VerifiOTP: FC<IVerifyOTPProps> = (props): JSX.Element => {
    const { loader, subHead = 'An OTP sent to mobile number XXXXXXXXXX', buttonConfig, isInputNum = true, numInputs = 6, otpDuration = 60, onSubmitData, resendOTPData } = props;
                        ...
                        <Box sx={{ textAlign: 'right', paddingTop: '.8rem' }}>
                            {counter === 0 && (
                                <Button onClick={otpResendHandler}>
                                    <Typography sx={{ textTransform: 'capitalize', fontSize: '.9rem' }}>Resend OTP</Typography>
                                </Button>
                            )}
                            {counter !== 0 && (
                                <Typography
